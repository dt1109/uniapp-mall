    ...
    <view style="position:relative;">
          ...
          <view>
            <view class="shopBuild"
              ...
              <image class="rightBlack"
                     :src="this.URLWAY.urlWay + '/icons/rightArowBlack.png'"
                     alt=""></image>
            </view>
      ...
      <view class="upData">
        <image style="margin-right:15rpx"
               :src="this.URLWAY.urlWay + '/icons/upArow.png'"
               alt=""></image>
        <text>上拉查看图文详情</text>
      </view>
      <view class="tuwenxiangqing"></view>
      <goods-detail :data="goodsData.goods.productDetailUrls"></goods-detail>
      <!-- 加入购物车 -->
        ...
        <view class="goods-choose-type">
          ...
          <view class="goods-choose">
            <view class="goods-choose-col">
              <image :src="goodsSpecObj.图片 || goodsInfo.banner[0]"
                     mode="" />
              <view style="padding-top:70rpx;margin-left:30rpx">
                <text><text class="goods-small">￥</text><text v-if="goodsSpecObj.现价">{{goodsSpecObj.现价}}</text><text
                        v-else-if="!goodsSpecObj.现价 && goodsData.goods.newprice">{{goodsData.goods.newprice}}</text></text>
                <view class="goods-kucun">库存：<text>{{goodsData.goods.stocks}}</text></view>
              </view>
            </view>
            <view v-for="(item, index) in goodsData.goods.specifications"
                  :key="index">
              <view class="goods-bold">
                {{ Object.keys(item)[0] }}
              </view>
              <view class="good-btn">
                <button :data-info="idx"
                        ...
                        v-for="(items, idx) in item[Object.keys(item)[0]]"
                        :key="idx"
                        size="mini"
                        @click="selectSpec(index, Object.keys(item)[0], items)"><text>{{items}}</text></button>
              </view>
            </view>
