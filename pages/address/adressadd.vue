<template>
	<view >
		<!-- <uni-nav-bar left-icon="back" backgroundColor='#e51c23' title="添加新地址" color='white' fixed="true" status-bar="true" ></uni-nav-bar> -->
		<view class="container">
		  <view class="address-management">新建收货地址</view>
		  ...
		  <view class="address-card" style="margin-top:30rpx">
		    <view>
		      设置默认地址
		      <switch @change="switchChange" color="#e51c23" style="float:right;transform:scale(0.7)" />
		    </view>
...
export default {
  ...
  methods: {
    ...
    handleGetRegion(region) {
      ...
      this.addressData.city = this.region[0].name
      this.addressData.area = this.region[0].name
    },
    // 添加收货地址
    addAddress() {
    ...
    getAddressArea() {
      ...
      console.log(value)
      this.regionName = [{name:value.province +  value.city +  value.area}]
