<template>
  <view style="background:#f7f7f7"
    ...
    <uni-nav-bar left-icon=""
                 ...
                 fixed="true"
                 @clickLeft="goBack"
                 @clickRight="goShare" status-bar="true" style="position:relative" v-if="scrollTopIndex > 100">
      <view style="text-align:center;width:100%"
            v-if="scrollTopIndex > 1000">图文详情</view>
      <view class="goodsFixview"
      ...
      <view slot="right">
        <image class="good-right-img"
               src='https://lztimg.oss-cn-qingdao.aliyuncs.com/home/img/icons/shareIcon.png'
               v-if="false">
        </image>
      </view>
    </uni-nav-bar>
	<uni-popup ref="popupghjk" type="bottom">
	    <uni-popup-share title="分享到" @select="select"></uni-popup-share>
	</uni-popup>
    <!-- 轮播图 -->
    <view class="container">
      <uni-swiper-dot :info="goodsInfo.banner"
                      :current="current"
                      field="content"
        ...
        <swiper class="goods-swiper"
                ...
                :duration="duration"
                @change="change">
          <swiper-item v-for="(item, index) in goodsInfo.banner"
                       :key="index">
            <view class="swiper-item uni-bg-red">
              <image class="good-swiper-img"
                     :src='item'></image>
            </view>
          </swiper-item>
        </swiper>
    ...
    <view style="position:relative;">
      ...
      <view class="goods-card goods-margin"
        ...
        <text class="goods-style"><text v-for="(item,index) in submitGoodsData.specList"
      ...
      <view class="goods-card goods-margin"
        ...
        <view class="goods-area">
          ...
          <view class="goods-style">{{addressDatacheckd.province}},{{addressDatacheckd.city}} <view class="goods-state"><text v-if="goodsData.goods.stocks >= 1">现货</text><text
                    v-if="goodsData.goods.stocks < 1">缺货</text></view>
          </view>
      ...
      <view class="goods-conditions">
        <view>
          <image src="https://lztimg.oss-cn-qingdao.aliyuncs.com/home/img/icons/guaranteeIcon.png"
                 mode="" />
          </image>
      ...
      <!-- 用户评价 -->
       <view @touchstart="touchstartClick" @touchmove="touchmoveClick" @touchend="touchendClick">
      <view class="goods-user-evaluate goods-card goods-margin">
        ...
        <view>
          <goods-eva :datas="storesNum.list"
                     :bottomType="1"></goods-eva>
        </view>
        <view class="goods-eva-more">
          <text class="goods-eva-moretext"
                v-if="storesNum.cmsCount > 0" @click.stop="showMoreEvaluate">查看更多评价</text>
          <text class="goods-eva-moretext"
                v-if="storesNum.cmsCount == 0">暂无评价</text>
        </view>
      </view>

      <!--店铺 -->
      <view class="goods-card goods-margin xiaozhuchaungyi">
        <view class="shopInfo">
          <image class="userBigPhoto"
                 :src="shopContent.store.showimg"
                 alt=""></image>
          <view>
            <view class="shopBuild"
                  @click="goShop">
              <text class="shopName">{{shopContent.store.name}}</text>
              <image class="rightBlack"
      ...
      <!-- 图文详情 -->
      <view class="upData">
        <image style="margin-right:15rpx"
      ...
      <uni-popup ref="popupArea"
        ...
        <view class="goods-choose-area">
          ...
          <view v-for="(item,index) in addressData" :key="index">
          <view class="goods-area-list"
                :class="index == addressDataindex ? 'addressactive' : ''"
                @click="changeAddress(index,item)" v-if="index < 3">
            <!-- <view  style="width:100%"> -->
              <image src="https://lztimg.oss-cn-qingdao.aliyuncs.com/home/img/icons/icon_goods_address.png"
                     class="img-location"></image>
              <text class="block-text"
                    style="margin-left:20rpx">{{item.province}}{{item.city}}{{item.area}}{{item.road}}</text>
              <image src="https://lztimg.oss-cn-qingdao.aliyuncs.com/home/img/icons/icon_pay_select.png"
                     :class="index == addressDataindex ? 'imgshow' : ''"
                     class="img-right imghide"></image>
            <!-- </view> -->
          </view>
          </view>
          <view class="goods-buynow">
            <view class="goos-buynow-btnsure">
              <text class="goos-buynow-btn btnred"
                    @click="chooseOtheraddress">选择其他收货地址</text>
            </view>
          </view>
        </view>
      </uni-popup>
	...
	export default {
		data() {
			return {
				...
				shopContent: {
					store: {
						showimg: 'http://lztimg.oss-cn-qingdao.aliyuncs.com/mall/images/20200615/dqy188.png',
						name: '指联旗舰店',
						goodsnum: '10086',
						followsNum: '12580'
					}
				...
				shareList: [{
						url: 'https://lztimg.oss-cn-qingdao.aliyuncs.com/static/h5/qq.png',
						title: 'QQ好友'
					},
					{
						url: 'https://lztimg.oss-cn-qingdao.aliyuncs.com/static/h5/weibo.png',
						title: '微博'
					},
					{
						url: 'https://lztimg.oss-cn-qingdao.aliyuncs.com/static/h5/pyq.png',
						title: '朋友圈'
					},
					{
						url: 'https://lztimg.oss-cn-qingdao.aliyuncs.com/static/h5/weix.png',
						title: '微信好友'
					},
					{
						url: 'https://lztimg.oss-cn-qingdao.aliyuncs.com/static/h5/clone.png',
						title: '复制链接'
					},
					{
						url: 'https://lztimg.oss-cn-qingdao.aliyuncs.com/static/h5/qqspace.png',
						title: 'QQ空间'
					},
					{
						url: 'https://lztimg.oss-cn-qingdao.aliyuncs.com/static/h5/xiangce.png',
						title: '保存相册'
					}
        ],
        shareUserId:"",
		...
		created() {
			this.$http({
				path: this.$o2oApi + '/wxauthorize/ticket',
				...
				wx.ready(function () {
					wx.updateAppMessageShareData({
						title: goodsData.goods.name, // 分享标题
						desc: '我在指乎发现一个不错的商品，赶快来看看吧。', // 分享描述
						link: window.location.href, // 分享链接
						imgUrl: this.goodsInfo.banner[0], // 分享图标
						success () {
							// 用户成功分享后执行的回调函数
							option.success()
							console.log('成功')
			...
			// wxShare({ title: 'title', url: window.location.href, img: this.goodsInfo.banner[0], desc: 'desc' })
			this.goodsId = GetUrlParam('goodsId')
		},
		onLoad(options) {
      //动态设置商品标题
      uni.setNavigationBarTitle({
          title: this.goodsData.goods.name
      });
      // console.log(options)
      if(options.goodsId){
        this.goodsId = options.goodsId
      }else{
          this.goodsId = '5e69fb59b8d6b92026761a0b'
      }
      if(options.sharingUserId){
        this.shareUserId = options.sharingUserId
      }else{
        this.shareUserId = ""
      }
			this.getGoodsInfo()
      ...
      showMoreEvaluate(){
           uni.navigateTo({
                url: '../../pages/goodsevaluate/index?goodsId='+this.goodsId+'&sharingUserId='+this.shareUserId
            })
      },
			// 获取所有收货地址
			getallArea() {
          ...
          uni.setStorageSync('goodsData', this.goodsData)
            ...
            uni.setStorageSync('goodsInfodata', this.goodsInfo)
					} else {
				...
				this.$http({
					...
					uni.navigateTo({
						url: '../../pages/order/index?id=' +
							this.goodsId +
							'&buynum=' +
							this.submitGoodsData.number +'&sharingUserId='+ this.shareUserId+
							''
					})
				})
			}
